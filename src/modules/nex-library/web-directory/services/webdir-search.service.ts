...
import { WebdirSearchResultDTO } from '../dtos/web-directory.dto';
...
@Injectable()
export class WebdirsearchService {
  private resultSize = 6;

  constructor(private readonly esService: ElasticsearchService) { }

  async indexWebdir(webdir: WebDirectory): Promise<Record<string, any>> {
    const exists: boolean = await this.checkIndexExists(webdir.uuid);

    if (!exists) {
      const data: WebDirectory[] = [];
      data.push(webdir);
      ...
      return this.esService.helpers.bulk({
        ...
        onDocument(doc) {
          return {
            index: { _index: EsIndex.WEB_DIRECTORIES_INDEX, _id: doc.uuid },
          };
        },
      });
    }
  ...
  async search(
    ...
    const exists = await this.esService.indices.exists({
      index: EsIndex.WEB_DIRECTORIES_INDEX,
    });
  ...
  async update(webdir: WebDirectory): Promise<Record<string, any>> {
    const exists: boolean = await this.checkIndexExists(webdir.uuid);

    if (!exists) return null;

    const payload: WebDirectory = { ...webdir };
    const data: WebDirectory[] = [];
    data.push(payload);
    ...
    return this.esService.helpers.bulk({
      ...
      onDocument(doc) {
        return [
          {
            update: {
              _index: EsIndex.WEB_DIRECTORIES_INDEX,
              _id: doc.uuid,
            },
          },
          { doc_as_upsert: false },
  ...
  private async initSearch(query: string, pit: OpenPointInTimeResponse) {
    return await this.esService.search<WebdirSearchResultDTO>({

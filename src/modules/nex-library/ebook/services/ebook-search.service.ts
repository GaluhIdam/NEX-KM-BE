...
import { EbookSearchResultDTO } from '../dtos/ebook.dto';
...
export class EbookSearchService {
  ...
  async indexEbook(ebook: Ebook): Promise<Record<string, any>> {
    const exists: boolean = await this.checkIndexExists(ebook.uuid);
    ...
    if (!exists) {
      ...
      const indexed = this.esService.helpers.bulk({
        ...
        onDocument(doc) {
          return {
            index: { _index: EsIndex.EBOOKS_INDEX, _id: doc.uuid },
          };
        },
        onDrop(doc) {
  ...
  async update(ebook: Ebook): Promise<Record<string, any>> {
    const exists: boolean = await this.checkIndexExists(ebook.uuid);

    if (!exists) return null;

    const payload: Ebook = { ...ebook };
    const data: Ebook[] = [];
    data.push(payload);
    ...
    return this.esService.helpers.bulk({
      ...
      onDocument(doc) {
        return [
          { update: { _index: EsIndex.EBOOKS_INDEX, _id: doc.uuid } },
          { doc_as_upsert: false },
        ];
      },
    });
  ...
  private async initSearch(query: string, pit: OpenPointInTimeResponse) {
    return await this.esService.search<EbookSearchResultDTO>({
      size: this.resultSize,
      query: {
        multi_match: {
          query,
          fields: ['title', 'synopsis', 'overview', 'author', 'aboutAuthor'],
          fuzziness: 'AUTO'
        },
      },
      pit: {
